...
inline fextl::string GetExecutablePath() {
#if defined(__APPLE__)
  char Path[PATH_MAX];
...
#else
  char Path[PATH_MAX];
  auto Result = readlink("/proc/self/exe", Path, PATH_MAX);
  if (Result == -1) {
    return {};
  }
  return fextl::string(Path, Result);
#endif
}
...
#if defined(__APPLE__)
    // macOS doesn't have sendfile with the same signature.
    // Use a simple read/write loop or copyfile() if available.
    char buffer[16384];
    ssize_t bytes_read;
    while ((bytes_read = read(SourceFD, buffer, sizeof(buffer))) > 0) {
      if (write(DestinationFD, buffer, bytes_read) != bytes_read) {
        close(DestinationFD);
        close(SourceFD);
        return false;
      }
    }
    bool Result = (bytes_read == 0);
#else
