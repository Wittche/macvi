...
#ifdef __APPLE__
  uint64_t GuestAddress = Address - FEXCore::Utils::GlobalMemoryBase;
#else
  uint64_t GuestAddress = Address;
#endif

  while (GuestAddress < ExecutableRangeBase || GuestAddress + Size > ExecutableRangeEnd) {
    auto RangeInfo = CTX->SyscallHandler->QueryGuestExecutableRange(Thread, GuestAddress);
    ...
    uint64_t RangeRemainingSize = ExecutableRangeEnd - GuestAddress;
    if (Size > RangeRemainingSize) {
      Size -= RangeRemainingSize;
      GuestAddress += RangeRemainingSize;
    }
  }
...
#if (defined(ASSERTIONS_ENABLED) && ASSERTIONS_ENABLED) || defined(__APPLE__)
#ifndef __APPLE__
...
void Decoder::DecodeInstructionsAtEntry(FEXCore::Core::InternalThreadState* Thread, const uint8_t* _InstStream, uint64_t PC, uint64_t MaxInst) {
  ...
  if (BlockInfo.Is64BitMode != CTX->Config.Is64BitMode) {
      fprintf(stderr, "[FEX JIT] Operating Mode Mismatch! CS_Idx: 0x%x, Segment.L: %d, Config.Is64Bit: %d\n", 
              Thread->CurrentFrame->State.cs_idx, CSSegment->L, (int)CTX->Config.Is64BitMode);
  }
...
#ifdef __APPLE__
  InstStream = reinterpret_cast<uint8_t*>(reinterpret_cast<uintptr_t>(_InstStream) + FEXCore::Utils::GlobalMemoryBase);
#else
...
#ifdef __APPLE__
    InstStream = reinterpret_cast<uint8_t*>(reinterpret_cast<uintptr_t>(AdjustAddrForSpecialRegion(_InstStream, EntryPoint, RIPToDecode)) + FEXCore::Utils::GlobalMemoryBase);
#else
