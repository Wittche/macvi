...
DEF_OP(ExitFunction) {
  ...
  if constexpr (Context::BLOCK_DEBUGGING) {
    // Skip block linking when BLOCK_DEBUGGING as it adds overhead and is unnecessary.
...
#ifdef __APPLE__
          ldr(TMP3, STATE_PTR(CpuStateFrame, State.callret_sp));
          stp<ARMEmitter::IndexType::PRE>(CallReturnAddressReg, TMP1, TMP3, -0x10);
          str(TMP3, STATE_PTR(CpuStateFrame, State.callret_sp));
#else
...
#ifdef __APPLE__
          ldr(TMP3, STATE_PTR(CpuStateFrame, State.callret_sp));
          stp<ARMEmitter::IndexType::PRE>(ARMEmitter::XReg::zr, ARMEmitter::XReg::zr, TMP3, -0x10);
          str(TMP3, STATE_PTR(CpuStateFrame, State.callret_sp));
#else
...
#ifdef __APPLE__
      ldr(TMP3, STATE_PTR(CpuStateFrame, State.callret_sp));
      ldp<ARMEmitter::IndexType::POST>(TMP1, TMP2, TMP3, 0x10);
      str(TMP3, STATE_PTR(CpuStateFrame, State.callret_sp));
#else
      ldp<ARMEmitter::IndexType::POST>(TMP1, TMP2, REG_CALLRET_SP, 0x10);
#endif
#ifdef __APPLE__
      (void)cbz(ARMEmitter::Size::i64Bit, RipReg.X(), &SkipFullLookup);
#endif
...
#ifdef __APPLE__
    ARMEmitter::ForwardLabel l_ReturnTargetValid;
    (void)cbnz(ARMEmitter::Size::i64Bit, TMP2, &l_ReturnTargetValid);
    ldr(TMP2, STATE, offsetof(FEXCore::Core::CpuStateFrame, Pointers.DispatcherLoopTop));
    str(RipReg.X(), STATE, offsetof(FEXCore::Core::CpuStateFrame, State.rip));
    (void)Bind(&l_ReturnTargetValid);
#endif
