...
struct RecentExitLinkRecord {
  uint64_t GuestRIP {};
  uint64_t HostCode {};
  uint32_t Instructions[6] {};
};

static std::array<RecentExitLinkRecord, 16> RecentExitLinks {};
static std::atomic<uint32_t> RecentExitLinksHead {};

static void RecordRecentExitLink(uint64_t GuestRIP, uint64_t HostCode) {
  auto Index = RecentExitLinksHead.fetch_add(1, std::memory_order_relaxed) % RecentExitLinks.size();
  auto &Entry = RecentExitLinks[Index];
  Entry.GuestRIP = GuestRIP;
  Entry.HostCode = HostCode;
  if (HostCode) {
    auto Code = reinterpret_cast<const uint32_t*>(HostCode);
    for (size_t i = 0; i < std::size(Entry.Instructions); ++i) {
      Entry.Instructions[i] = Code[i];
    }
  } else {
    memset(Entry.Instructions, 0, sizeof(Entry.Instructions));
  }
}
...
void DumpRecentExitLinks() {
  auto Head = RecentExitLinksHead.load(std::memory_order_relaxed);
  fprintf(stderr, "[FEXDBG] Recent ExitFunctionLink targets:\n");
  for (size_t i = 0; i < RecentExitLinks.size(); ++i) {
    auto Index = (Head + i) % RecentExitLinks.size();
    const auto &Entry = RecentExitLinks[Index];
    if (!Entry.GuestRIP && !Entry.HostCode) {
      continue;
    }
    fprintf(stderr, "  guest=0x%llx host=0x%llx inst=%08x %08x %08x %08x %08x %08x\n",
            static_cast<unsigned long long>(Entry.GuestRIP),
            static_cast<unsigned long long>(Entry.HostCode),
            Entry.Instructions[0], Entry.Instructions[1], Entry.Instructions[2],
            Entry.Instructions[3], Entry.Instructions[4], Entry.Instructions[5]);
  }
}
} // namespace FEXCore::CPU
...
uint64_t Arm64JITCore::ExitFunctionLink(FEXCore::Core::CpuStateFrame* Frame, FEXCore::Context::ExitFunctionLinkData* Record) {
  ...
  // ALWAYS update Frame->State.rip to the branch target.
  // This ensures that if a crash occurs during CompileBlock (e.g., GuestRip is invalid),
  // the crash dump will correctly identify the target RIP.
  Frame->State.rip = GuestRip;

#ifdef __APPLE__
  if (GuestRip >= 0x4bf000 && GuestRip < 0x4d0000) {
    // #region agent log
    FEXCore::DebugSession::Log("JIT.cpp:ExitFunctionLink", "B", "exit_link_enter", GuestRip,
                               Frame->State.gregs[X86State::REG_R14], 0, Record->HostCode);
    // #endregion
  }
#endif
  ...
  if (TFSet) {
    ...
    if (!HostCode) {
      ...
      if (HostCode == 0) {
        fprintf(stderr, "[FEX] ExitFunctionLink: CompileBlock returned 0 for GuestRip=0x%llx\n", (unsigned long long)GuestRip);
        fflush(stderr);
      }
  ...
  RecordRecentExitLink(GuestRip, HostCode);
  ...
  if (GuestRip >= 0x4bf000 && GuestRip < 0x4d0000) {
    // #region agent log
    FEXCore::DebugSession::Log("JIT.cpp:ExitFunctionLink", "B", "exit_link_return", GuestRip,
                               Frame->State.gregs[X86State::REG_R14], 0, HostCode);
    // #endregion
  }
#endif
