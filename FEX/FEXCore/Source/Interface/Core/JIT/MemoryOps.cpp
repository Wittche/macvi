...
#ifdef __APPLE__
// Helper macros to apply GlobalMemoryBase shifting for macOS.
// Safely handles collisions where HostReg or GuestReg might be TmpReg.
// IMPORTANT: We use cbz instead of cmp to avoid clobbering host NZCV flags.
#define SHIFT_TO_HOST_SAFE(HostReg, GuestReg, TmpReg) \
  do { \
    if (FEXCore::Utils::GlobalMemoryBase != 0) { \
      ARMEmitter::Register _ShiftTmp = TmpReg; \
      if (GuestReg == TmpReg || HostReg == TmpReg) { \
         _ShiftTmp = TMP4; \
      } \
...
#define UNSHIFT_FROM_HOST_SAFE(GuestReg, HostReg, TmpReg) \
  do { \
    if (FEXCore::Utils::GlobalMemoryBase != 0) { \
      ARMEmitter::Register _ShiftTmp = TmpReg; \
      if (HostReg == TmpReg || GuestReg == TmpReg) { \
         _ShiftTmp = TMP4; \
      } \
      ARMEmitter::ForwardLabel _ZeroLabel; \
      if (GuestReg != HostReg) { \
        mov(ARMEmitter::Size::i64Bit, GuestReg, HostReg); \
      } \
      (void)cbz(ARMEmitter::Size::i64Bit, HostReg, &_ZeroLabel); \
      LoadConstant(ARMEmitter::Size::i64Bit, _ShiftTmp, FEXCore::Utils::GlobalMemoryBase); \
      sub(ARMEmitter::Size::i64Bit, GuestReg, HostReg, _ShiftTmp); \
      (void)Bind(&_ZeroLabel); \
    } else { \
      mov(ARMEmitter::Size::i64Bit, GuestReg, HostReg); \
    } \
...
ARMEmitter::ExtendedMemOperand Arm64JITCore::GenerateMemOperand(
  ...
  ARMEmitter::Register FinalBase = Base;

#ifdef __APPLE__
  if (FEXCore::Utils::GlobalMemoryBase != 0) {
    ARMEmitter::Register _ShiftTmp = TMP2;
    if (Base == TMP2) {
      _ShiftTmp = TMP4;
    }
    
    // macOS 16k compatibility: NULL pointer must remain NULL.
    // We use a conditional branch to avoid clobbering NZCV flags with cmp.
    ARMEmitter::ForwardLabel _ZeroLabel;
    (void)cbz(ARMEmitter::Size::i64Bit, Base, &_ZeroLabel);
    
    LoadConstant(ARMEmitter::Size::i64Bit, _ShiftTmp, FEXCore::Utils::GlobalMemoryBase);
    add(ARMEmitter::Size::i64Bit, _ShiftTmp, Base, _ShiftTmp);
    FinalBase = _ShiftTmp;
  ...
  if (Offset.IsInvalid()) {
    return ARMEmitter::ExtendedMemOperand(FinalBase.X(), ARMEmitter::IndexType::OFFSET, 0);
  } else {
    ...
    if (IsInlineConstant(Offset, &Const)) {
      return ARMEmitter::ExtendedMemOperand(FinalBase.X(), ARMEmitter::IndexType::OFFSET, Const);
    } else {
      auto RegOffset = GetReg(Offset);
      switch (OffsetType) {
      case IR::MemOffsetType::SXTX:
        return ARMEmitter::ExtendedMemOperand(FinalBase.X(), RegOffset.X(), ARMEmitter::ExtendedType::SXTX, FEXCore::ilog2(OffsetScale));
      case IR::MemOffsetType::UXTW:
        return ARMEmitter::ExtendedMemOperand(FinalBase.X(), RegOffset.X(), ARMEmitter::ExtendedType::UXTW, FEXCore::ilog2(OffsetScale));
      case IR::MemOffsetType::SXTW:
        return ARMEmitter::ExtendedMemOperand(FinalBase.X(), RegOffset.X(), ARMEmitter::ExtendedType::SXTW, FEXCore::ilog2(OffsetScale));
...
ARMEmitter::Register Arm64JITCore::ApplyMemOperand(IR::OpSize AccessSize, ARMEmitter::Register Base, ARMEmitter::Register Tmp,
  ...
  ARMEmitter::Register FinalBase = Base;
#ifdef __APPLE__
  SHIFT_TO_HOST_SAFE(Tmp, Base, TMP3);
  FinalBase = Tmp;
#endif
  if (Offset.IsInvalid()) {
    return FinalBase;
  ...
  if (IsInlineConstant(Offset, &Const)) {
    ...
    add(ARMEmitter::Size::i64Bit, Tmp, FinalBase, Tmp, ARMEmitter::ShiftType::LSL, FEXCore::ilog2(OffsetScale));
  } else {
    auto RegOffset = GetReg(Offset);
    switch (OffsetType) {
    case IR::MemOffsetType::SXTX:
      add(ARMEmitter::Size::i64Bit, Tmp, FinalBase, RegOffset, ARMEmitter::ExtendedType::SXTX, FEXCore::ilog2(OffsetScale));
      break;

    case IR::MemOffsetType::UXTW:
      add(ARMEmitter::Size::i64Bit, Tmp, FinalBase, RegOffset, ARMEmitter::ExtendedType::UXTW, FEXCore::ilog2(OffsetScale));
      break;

    case IR::MemOffsetType::SXTW:
      add(ARMEmitter::Size::i64Bit, Tmp, FinalBase, RegOffset, ARMEmitter::ExtendedType::SXTW, FEXCore::ilog2(OffsetScale));
      break;
...
ARMEmitter::SVEMemOperand Arm64JITCore::GenerateSVEMemOperand(IR::OpSize AccessSize, ARMEmitter::Register Base, IR::OrderedNodeWrapper Offset,
  ...
  ARMEmitter::Register FinalBase = Base;
#ifdef __APPLE__
  SHIFT_TO_HOST_SAFE(TMP1, Base, TMP1);
  FinalBase = TMP1;
#endif

  if (Offset.IsInvalid()) {
    return ARMEmitter::SVEMemOperand(FinalBase.X(), 0);
  }
  uint64_t Const {};
  if (IsInlineConstant(Offset, &Const)) {
    if (Const == 0) {
      // BUG FIX: Was using Base (guest addr) instead of FinalBase (host addr).
      // This caused GlobalMemoryBase to be completely bypassed for SVE ops with zero offset.
      return ARMEmitter::SVEMemOperand(FinalBase.X(), 0);
    }
    ...
    if (IsCleanlyDivisible && Index >= -8 && Index <= 7) {
      return ARMEmitter::SVEMemOperand(FinalBase.X(), static_cast<uint64_t>(Index));
    }

    // If we can't do that for whatever reason, then unfortunately, we need
    // to move it over to a temporary to use as an offset.
    // BUG FIX: Was using TMP1 which IS FinalBase on macOS, destroying the shifted address.
    // Use TMP3 instead to avoid clobbering the base register.
    mov(TMP3, Const);
    return ARMEmitter::SVEMemOperand(FinalBase.X(), TMP3);
  }
  ...
  return ARMEmitter::SVEMemOperand(FinalBase.X(), RegOffset.X());
}
...
DEF_OP(LoadMemPair) {
...
#ifdef __APPLE__
  ARMEmitter::Register HostAddr = TMP2;
  SHIFT_TO_HOST_SAFE(HostAddr, Addr, TMP2);
#else
  const auto HostAddr = Addr;
#endif
  ...
  if (Op->Class == IR::RegClass::GPR) {
    ...
    switch (IROp->Size) {
    case IR::OpSize::i32Bit: ldp<ARMEmitter::IndexType::OFFSET>(Dst1.W(), Dst2.W(), HostAddr, Op->Offset); break;
    case IR::OpSize::i64Bit: ldp<ARMEmitter::IndexType::OFFSET>(Dst1.X(), Dst2.X(), HostAddr, Op->Offset); break;
    default: LOGMAN_MSG_A_FMT("Unhandled LoadMemPair size: {}", IROp->Size); break;
    }
  } else {
    const auto Dst1 = GetVReg(Op->OutValue1);
    ...
    switch (IROp->Size) {
    case IR::OpSize::i32Bit: ldp<ARMEmitter::IndexType::OFFSET>(Dst1.S(), Dst2.S(), HostAddr, Op->Offset); break;
    case IR::OpSize::i64Bit: ldp<ARMEmitter::IndexType::OFFSET>(Dst1.D(), Dst2.D(), HostAddr, Op->Offset); break;
    case IR::OpSize::i128Bit: ldp<ARMEmitter::IndexType::OFFSET>(Dst1.Q(), Dst2.Q(), HostAddr, Op->Offset); break;
    default: LOGMAN_MSG_A_FMT("Unhandled LoadMemPair size: {}", IROp->Size); break;
    }
  }
}
...
DEF_OP(LoadMemTSO) {
...
#ifdef __APPLE__
  ARMEmitter::Register AccessReg = TMP2;
  SHIFT_TO_HOST_SAFE(AccessReg, MemReg, TMP2);
#else
  const auto AccessReg = MemReg;
#endif
  ...
  if (CTX->HostFeatures.SupportsTSOImm9 && Op->Class == IR::RegClass::GPR) {
    ...
    if (OpSize == IR::OpSize::i8Bit) {
      ...
      const auto Dst = GetReg(Node);
      ldapurb(Dst, AccessReg, Offset);
    } else {
      switch (OpSize) {
      case IR::OpSize::i16Bit: ldapurh(Dst, AccessReg, Offset); break;
      case IR::OpSize::i32Bit: ldapur(Dst.W(), AccessReg, Offset); break;
      case IR::OpSize::i64Bit: ldapur(Dst.X(), AccessReg, Offset); break;
    ...
    if (OpSize == IR::OpSize::i8Bit) {
      // 8bit load is always aligned to natural alignment
      ldaprb(Dst.W(), AccessReg);
    } else {
      switch (OpSize) {
      case IR::OpSize::i16Bit: ldaprh(Dst.W(), AccessReg); break;
      case IR::OpSize::i32Bit: ldapr(Dst.W(), AccessReg); break;
      case IR::OpSize::i64Bit: ldapr(Dst.X(), AccessReg); break;
      default: LOGMAN_MSG_A_FMT("Unhandled LoadMemTSO size: {}", OpSize); break;
      }
      // Half-barrier once back-patched.
      nop();
    ...
    if (OpSize == IR::OpSize::i8Bit) {
      ...
      switch (OpSize) {
      case IR::OpSize::i16Bit: ldarh(Dst, AccessReg); break;
      case IR::OpSize::i32Bit: ldar(Dst.W(), AccessReg); break;
      case IR::OpSize::i64Bit: ldar(Dst.X(), AccessReg); break;
      default: LOGMAN_MSG_A_FMT("Unhandled LoadMemTSO size: {}", OpSize); break;
      }
      // Half-barrier once back-patched.
      nop();
...
#ifdef __APPLE__
    if (FEXCore::Utils::GlobalMemoryBase != 0) {
      auto WorkingBase = TempMemReg;
      TempMemReg = TMP4;
      LoadConstant(ARMEmitter::Size::i64Bit, TMP3, FEXCore::Utils::GlobalMemoryBase);
      add(ARMEmitter::Size::i64Bit, TempMemReg, WorkingBase, TMP3);
      cmp(ARMEmitter::Size::i64Bit, WorkingBase, 0);
      csel(ARMEmitter::Size::i64Bit, TempMemReg, TempMemReg, WorkingBase, ARMEmitter::Condition::CC_NE);
    }
#endif
...
#ifdef __APPLE__
  ARMEmitter::Register HostMemoryBaseReg = TMP4;
  if (FEXCore::Utils::GlobalMemoryBase != 0) {
    if (TempMemReg == TMP4 || WorkingReg == TMP4) {
      // Collision with TMP4! We need another temp. TMP3 is usually safe here.
      HostMemoryBaseReg = TMP3;
    }
    LoadConstant(ARMEmitter::Size::i64Bit, HostMemoryBaseReg, FEXCore::Utils::GlobalMemoryBase);
  }
#endif
...
#ifdef __APPLE__
    if (FEXCore::Utils::GlobalMemoryBase != 0) {
      ARMEmitter::ForwardLabel SkipShift;
      cbz(ARMEmitter::Size::i64Bit, TempMemReg, &SkipShift);
      add(ARMEmitter::Size::i64Bit, TempMemReg, TempMemReg, HostMemoryBaseReg);
      Bind(&SkipShift);
    }
#endif
...
#ifdef __APPLE__
      SHIFT_TO_HOST_SAFE(AddrReg, AddrReg, TMP2);
#endif
...
#ifdef __APPLE__
    ARMEmitter::Register HostMemReg = TMP2;
    SHIFT_TO_HOST_SAFE(HostMemReg, MemReg, TMP2);
#else
    const auto& HostMemReg = MemReg;
#endif

    switch (ElementSize) {
    case IR::OpSize::i8Bit: ld1<ARMEmitter::SubRegSize::i8Bit>(Dst.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i16Bit: ld1<ARMEmitter::SubRegSize::i16Bit>(Dst.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i32Bit: ld1<ARMEmitter::SubRegSize::i32Bit>(Dst.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i64Bit: ld1<ARMEmitter::SubRegSize::i64Bit>(Dst.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i128Bit: ldr(Dst.Q(), HostMemReg); break;
...
DEF_OP(VStoreVectorElement) {
  ...
  if (Is256Bit) {
    ...
    switch (ElementSize) {
    case IR::OpSize::i8Bit: st1<ARMEmitter::SubRegSize::i8Bit>(Value.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i16Bit: st1<ARMEmitter::SubRegSize::i16Bit>(Value.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i32Bit: st1<ARMEmitter::SubRegSize::i32Bit>(Value.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i64Bit: st1<ARMEmitter::SubRegSize::i64Bit>(Value.Q(), Op->Index, HostMemReg); break;
    case IR::OpSize::i128Bit: str(Value.Q(), HostMemReg); break;
    default: LOGMAN_MSG_A_FMT("Unhandled {} size: {}", __func__, ElementSize); return;
    }
  }
}
...
DEF_OP(VBroadcastFromMem) {
  ...
  if (Is256Bit && HostSupportsSVE256) {
    ...
    switch (ElementSize) {
    case IR::OpSize::i8Bit: ld1rb(ARMEmitter::SubRegSize::i8Bit, Dst.Z(), GoverningPredicate, HostMemReg); break;
    case IR::OpSize::i16Bit: ld1rh(ARMEmitter::SubRegSize::i16Bit, Dst.Z(), GoverningPredicate, HostMemReg); break;
    case IR::OpSize::i32Bit: ld1rw(ARMEmitter::SubRegSize::i32Bit, Dst.Z(), GoverningPredicate, HostMemReg); break;
    case IR::OpSize::i64Bit: ld1rd(Dst.Z(), GoverningPredicate, HostMemReg); break;
    case IR::OpSize::i128Bit: ld1rqb(Dst.Z(), GoverningPredicate, HostMemReg); break;
    default: LOGMAN_MSG_A_FMT("Unhandled VBroadcastFromMem size: {}", ElementSize); return;
    }
  } else {
    switch (ElementSize) {
    case IR::OpSize::i8Bit: ld1r<ARMEmitter::SubRegSize::i8Bit>(Dst.Q(), HostMemReg); break;
    case IR::OpSize::i16Bit: ld1r<ARMEmitter::SubRegSize::i16Bit>(Dst.Q(), HostMemReg); break;
    case IR::OpSize::i32Bit: ld1r<ARMEmitter::SubRegSize::i32Bit>(Dst.Q(), HostMemReg); break;
    case IR::OpSize::i64Bit: ld1r<ARMEmitter::SubRegSize::i64Bit>(Dst.Q(), HostMemReg); break;
    case IR::OpSize::i128Bit:
      // Normal load, like ld1rqb with 128-bit regs.
      ldr(Dst.Q(), HostMemReg);
      break;
    default: LOGMAN_MSG_A_FMT("Unhandled VBroadcastFromMem size: {}", ElementSize); return;
    }
  }
...
DEF_OP(Push) {
  ...
  #ifdef __APPLE__
  ARMEmitter::Register StoreBase = TMP3;
  SHIFT_TO_HOST_SAFE(StoreBase, AddrSrc, TMP2);
  #else
  auto StoreBase = AddrSrc;
  #endif

  switch (ValueSize) {
  case 1: {
    sturb(Src.W(), StoreBase, -ValueSize);
    break;
  }
  case 2: {
    sturh(Src.W(), StoreBase, -ValueSize);
    break;
  }
  case 4: {
    stur(Src.W(), StoreBase, -ValueSize);
    break;
  }
  case 8: {
    stur(Src.X(), StoreBase, -ValueSize);
    break;
  }
  default: {
  ...
  // Result SP stays in guest address space (no GlobalMemoryBase).
...
DEF_OP(PushTwo) {
  ...
  printf("PushTwo IROp->Size: %d, ValueSize: %d\n", (uint32_t)IROp->Size, ValueSize);
  fflush(stdout);
  ...
  // If Src1 or Src2 alias AddrSrc, move to temporary first.
  if (Src1 == AddrSrc) {
    mov(TMP1, Src1.X());
    Src1 = TMP1;
  }
  ...
  switch (ValueSize) {
  case 4: {
    stur(Src1.W(), StoreBase, -2 * ValueSize);
    stur(Src2.W(), StoreBase, -1 * ValueSize);
    break;
  }
  case 8: {
    stur(Src1.X(), StoreBase, -2 * ValueSize);
    stur(Src2.X(), StoreBase, -1 * ValueSize);
    break;
  }
  default: {
    LOGMAN_MSG_A_FMT("Unhandled {} size: {}", __func__, ValueSize);
  ...
  // Result SP stays in guest address space (no GlobalMemoryBase).
  sub(IROp->Size == IR::OpSize::i64Bit ? ARMEmitter::Size::i64Bit : ARMEmitter::Size::i32Bit, Dst, AddrSrc, 2 * ValueSize);
}
...
DEF_OP(Pop) {
...
#ifdef __APPLE__
  ARMEmitter::Register LoadBase = TMP3;
  SHIFT_TO_HOST_SAFE(LoadBase, Addr, TMP2);
#else
  auto LoadBase = Addr;
#endif

  LOGMAN_THROW_A_FMT(Dst != Addr, "Invalid: RA should not assign same register for Addr and Dst in Pop");

  switch (Size) {
  case 1: {
    ldrb(Dst.W(), LoadBase, 0);
    break;
  }
  case 2: {
    ldrh(Dst.W(), LoadBase, 0);
    break;
  }
  case 4: {
    ldr(Dst.W(), LoadBase, 0);
    break;
  }
  case 8: {
    ldr(Dst.X(), LoadBase, 0);
    break;
  }
  default: {
  ...
  // Update stack pointer guest register explicitly
...
DEF_OP(PopTwo) {
  ...
  LOGMAN_THROW_A_FMT(Dst1 != Addr && Dst2 != Addr, "Invalid: RA should not assign same register for Addr and Dst in PopTwo");
  ...
  switch (Size) {
  case 4: {
    ldp<ARMEmitter::IndexType::OFFSET>(Dst1.W(), Dst2.W(), LoadBase, 0);
    break;
  }
  case 8: {
    ldp<ARMEmitter::IndexType::OFFSET>(Dst1.X(), Dst2.X(), LoadBase, 0);
    break;
  }
  default: {
...
DEF_OP(StoreMemX87SVEOptPredicate) {
  ...
  ARMEmitter::Register AccessReg = TMP2;
  SHIFT_TO_HOST_SAFE(AccessReg, MemReg, TMP2);
  const auto MemDst = ARMEmitter::SVEMemOperand(AccessReg.X(), 0);
#else
...
DEF_OP(StoreMemPair) {
  ...
  if (Op->Class == IR::RegClass::GPR) {
    ...
    switch (OpSize) {
    case IR::OpSize::i32Bit: stp<ARMEmitter::IndexType::OFFSET>(Src1.W(), Src2.W(), HostAddr, Op->Offset); break;
    case IR::OpSize::i64Bit: stp<ARMEmitter::IndexType::OFFSET>(Src1.X(), Src2.X(), HostAddr, Op->Offset); break;
    default: LOGMAN_MSG_A_FMT("Unhandled StoreMem size: {}", OpSize); break;
    }
  } else {
    const auto Src1 = GetVReg(Op->Value1);
    ...
    switch (OpSize) {
    case IR::OpSize::i32Bit: stp<ARMEmitter::IndexType::OFFSET>(Src1.S(), Src2.S(), HostAddr, Op->Offset); break;
    case IR::OpSize::i64Bit: stp<ARMEmitter::IndexType::OFFSET>(Src1.D(), Src2.D(), HostAddr, Op->Offset); break;
    case IR::OpSize::i128Bit: stp<ARMEmitter::IndexType::OFFSET>(Src1.Q(), Src2.Q(), HostAddr, Op->Offset); break;
...
DEF_OP(StoreMemTSO) {
  ...
  if (CTX->HostFeatures.SupportsTSOImm9 && Op->Class == IR::RegClass::GPR) {
    ...
    if (OpSize == IR::OpSize::i8Bit) {
      // 8bit load is always aligned to natural alignment
      stlurb(Src, AccessReg, Offset);
    } else {
      // Half-barrier once back-patched.
      nop();
      switch (OpSize) {
      case IR::OpSize::i16Bit: stlurh(Src, AccessReg, Offset); break;
      case IR::OpSize::i32Bit: stlur(Src.W(), AccessReg, Offset); break;
      case IR::OpSize::i64Bit: stlur(Src.X(), AccessReg, Offset); break;
    ...
    if (OpSize == IR::OpSize::i8Bit) {
      ...
      nop();
      switch (OpSize) {
      case IR::OpSize::i16Bit: stlrh(Src, AccessReg); break;
      case IR::OpSize::i32Bit: stlr(Src.W(), AccessReg); break;
      case IR::OpSize::i64Bit: stlr(Src.X(), AccessReg); break;
      default: LOGMAN_MSG_A_FMT("Unhandled StoreMemTSO size: {}", OpSize); break;
      }
    }
  } else {
...
#ifdef __APPLE__
  SHIFT_TO_HOST_SAFE(TMP2, TMP2, TMP3);
#endif
...
#ifdef __APPLE__
  SHIFT_TO_HOST_SAFE(TMP2, TMP2, TMP4);
  SHIFT_TO_HOST_SAFE(TMP3, TMP3, TMP4);
#endif
...
DEF_OP(CacheLineClear) {
...
#ifdef __APPLE__
  SHIFT_TO_HOST_SAFE(MemReg, MemReg, TMP2);
#endif
  ...
  if (CTX->HostFeatures.DCacheLineSize >= 64U) {
    ...
    for (size_t i = 0; i < std::max(1U, CTX->HostFeatures.DCacheLineSize / 64U); ++i) {
      dc(ARMEmitter::DataCacheOperation::CIVAC, MemReg);
      add(ARMEmitter::Size::i64Bit, MemReg, MemReg, CTX->HostFeatures.DCacheLineSize);
    }
  }
...
DEF_OP(VStoreNonTemporal) {
  ...
  if (Is256Bit) {
    ...
    stnt1b(Value.Z(), GoverningPredicate, HostMemReg, OffsetScaled);
...
DEF_OP(VStoreNonTemporalPair) {
  ...
  stnp(ValueLow.Q(), ValueHigh.Q(), HostMemReg, Offset);
}
...
DEF_OP(VLoadNonTemporal) {
  ...
  if (Is256Bit) {
    const auto GoverningPredicate = PRED_TMP_32B.Zeroing();
    const auto OffsetScaled = Offset / 32;
    ldnt1b(Dst.Z(), GoverningPredicate, HostMemReg, OffsetScaled);
  } else if (Is128Bit && HostSupportsSVE128) {
    const auto GoverningPredicate = PRED_TMP_16B.Zeroing();
    const auto OffsetScaled = Offset / 16;
    ldnt1b(Dst.Z(), GoverningPredicate, HostMemReg, OffsetScaled);
