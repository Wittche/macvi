...
Dispatcher::Dispatcher(FEXCore::Context::ContextImpl* ctx)
  ...
  , CTX {ctx} {
  fprintf(stderr, "[Dispatcher] Constructor: CTX=%p, Buffer=%p, Size=0x%zx\n", ctx, GetBufferBase(), MAX_DISPATCHER_CODE_SIZE);
#ifdef __APPLE__
  pthread_jit_write_protect_np(false);
#endif
  LogMan::Msg::DFmt("Dispatcher: Starting emission...");
  EmitDispatcher();
  LogMan::Msg::DFmt("Dispatcher: Emission finished.");
#ifdef __APPLE__
...
void Dispatcher::EmitDispatcher() {
  ...
  // Need to create the block
  {
    ...
    ARMEmitter::ForwardLabel CompileFailed;
    cbz(ARMEmitter::Size::i64Bit, ARMEmitter::Reg::r0, &CompileFailed);

    // Jump to the compiled block
    br(ARMEmitter::XReg::x0);

    Bind(&CompileFailed);
    str(ARMEmitter::XReg::x0, ARMEmitter::XReg::x0, 0);
  }
...

  {
    ...
    if (CTX->ExitOnHLTEnabled()) {
      SpillStaticRegs(TMP1); // Save registers (e.g. RAX) before exiting!
...

  {
    ...
    if (FEXCore::Utils::GlobalMemoryBase != 0) {
      LoadConstant(ARMEmitter::Size::i64Bit, TMP4, FEXCore::Utils::GlobalMemoryBase);
      add(ARMEmitter::Size::i64Bit, ARMEmitter::Reg::r2, ARMEmitter::Reg::r2, TMP4, ARMEmitter::ShiftType::LSL, 0);
    }
#endif
    ...
    FillStaticRegs();
#ifdef __APPLE__
    ldr(TMP1, STATE_PTR(CpuStateFrame, State.callret_sp));
    stp<ARMEmitter::IndexType::PRE>(ARMEmitter::XReg::zr, ARMEmitter::XReg::zr, TMP1, -0x10);
    str(TMP1, STATE_PTR(CpuStateFrame, State.callret_sp));
#else
...
void Dispatcher::ExecuteDispatch(FEXCore::Core::CpuStateFrame* Frame) {
  fprintf(stderr, "FEX: Entering JIT Dispatcher\n");
  fflush(stderr);
...
#ifdef __APPLE__
    fprintf(stderr,
            "FEXDBG_DISPATCHER: InitThreadPointers DispatcherLoopTop=0x%llx "
            "SignalReturnHandler=0x%llx SignalReturnHandlerRT=0x%llx ThunkCallbackRet=0x%llx "
            "GuestSignal_SIGSEGV=0x%llx GlobalMemoryBase=0x%llx\n",
            static_cast<unsigned long long>(Ptrs.DispatcherLoopTop),
            static_cast<unsigned long long>(Ptrs.SignalReturnHandler),
            static_cast<unsigned long long>(Ptrs.SignalReturnHandlerRT),
            static_cast<unsigned long long>(Ptrs.ThunkCallbackRet),
            static_cast<unsigned long long>(Ptrs.GuestSignal_SIGSEGV),
            static_cast<unsigned long long>(FEXCore::Utils::GlobalMemoryBase));
#endif
  }
}
