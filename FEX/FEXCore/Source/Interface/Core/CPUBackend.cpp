...
namespace CPU {
  ...
  CodeBuffer::CodeBuffer(size_t Size)
    ...
    if (Size > HostPageSize) {
      ...
      if (!FEXCore::Allocator::VirtualProtect(reinterpret_cast<void*>(LastPageAddr), HostPageSize,
...
#define PR_GET_MDWE 66
  ...
  bool CPUBackend::IsAddressInCodeBuffer(uintptr_t Address) const {
    ...
    if (CurrentCodeBuffer && CheckCodeBuffer(*CurrentCodeBuffer, Address)) {
      return true;
    }
    ...
    if (getenv("FEX_DEBUG_SIGILL")) {
      fprintf(stderr, "CPUBackend::IsAddressInCodeBuffer: PC 0x%lx NOT found. Current: [0x%lx, 0x%lx), %zu buffers checked.\n",
              Address, (uintptr_t)(CurrentCodeBuffer ? CurrentCodeBuffer->Ptr : 0), 
              (uintptr_t)(CurrentCodeBuffer ? (CurrentCodeBuffer->Ptr + CurrentCodeBuffer->AllocatedSize) : 0),
              SignalHandlerCodeBuffers.size());
    }
    return false;
  }
