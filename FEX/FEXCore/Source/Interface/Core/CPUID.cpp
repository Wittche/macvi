...
void CPUIDEmu::SetupHostHybridFlag() {
  ...
  uint64_t MIDR {};
#ifdef __APPLE__
  static char BrandString[256] = "";
  if (BrandString[0] == '\0') {
    size_t size = sizeof(BrandString);
    if (sysctlbyname("machdep.cpu.brand_string", BrandString, &size, nullptr, 0) != 0) {
      strncpy(BrandString, "Apple Silicon", sizeof(BrandString));
    }
  }
  const char* DefaultARMProductName = BrandString;
#else
  const char* DefaultARMProductName = ProductNames::ARM_UNKNOWN;
#endif
  ...
  for (size_t i = 0; i < Cores; ++i) {
    ...
    PerCPUData[i].ProductName = DefaultARMProductName;
  ...
  static constexpr std::array<CPUMIDR, 68> CPUMIDRs = {{
    ...
    {0x51, 0x805, 0, ProductNames::ARM_Kryo400},   // Kryo 4xx Silver (A55 based)
    {0x51, 0x803, 0, ProductNames::ARM_Kryo300},   // Kryo 3xx Silver (A55 based)
    {0x51, 0x801, 0, ProductNames::ARM_Kryo200},   // Kryo 2xx Silver (A55 based)
    ...
    {0x61, 0x036, 0, ProductNames::ARM_Blizzard_M2Pro}, // Apple Blizzard (M2 Pro)
    {0x61, 0x03a, 0, ProductNames::ARM_Blizzard_M2Max}, // Apple Blizzard (M2 Max)
    {0x61, 0x022, 0, ProductNames::ARM_Icestorm_M1},    // Apple Icestorm (M1)
    {0x61, 0x026, 0, ProductNames::ARM_Icestorm_M1Pro}, // Apple Icestorm (M1 Pro)
    {0x61, 0x02a, 0, ProductNames::ARM_Icestorm_M1Max}, // Apple Icestorm (M1 Max)
  ...
  if (Hybrid) {
    ...
    for (auto& Data : PerCPUData) {
      ...
      Data.IsBig = FoundBig;
      if (MIDR && MIDR->ProductName && strcmp(MIDR->ProductName, ProductNames::ARM_UNKNOWN) != 0) {
        Data.ProductName = MIDR->ProductName;
      } else {
        Data.ProductName = DefaultARMProductName;
      }
    }
  } else {
    ...
    for (size_t i = 0; i < Cores; ++i) {
      ...
      if (MIDROption && MIDROption->ProductName && strcmp(MIDROption->ProductName, ProductNames::ARM_UNKNOWN) != 0) {
