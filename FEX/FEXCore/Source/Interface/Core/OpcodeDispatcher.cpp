...
void OpDispatchBuilder::LEAOp(OpcodeArgs) {
  ...
  auto Src = LoadSourceGPR_WithOpSize(Op, Op->Src[0], SrcSize, Op->Flags, {.LoadData = false, .AllowUpperGarbage = SrcSize > DstSize, .AddMemoryBase = false});
...
AddressMode OpDispatchBuilder::DecodeAddress(const X86Tables::DecodedOp& Op, const X86Tables::DecodedOperand& Operand,
                                              MemoryAccessType AccessType, bool IsLoad) {  const auto GPRSize = GetGPROpSize();
...
Ref OpDispatchBuilder::LoadSource_WithOpSize(RegClass Class, const X86Tables::DecodedOp& Op, const X86Tables::DecodedOperand& Operand,
  ...
  auto [Align, LoadData, ForceLoad, AccessType, AllowUpperGarbage, AddMemoryBase] = Options;
  ...
  if (Operand.IsGPR()) {
    ...
    if (gpr >= FEXCore::X86State::REG_MM_0) {
      ...
      if (!AllowUpperGarbage && OpSize < OpSize::i128Bit) {
        Result = _VMov(OpSize, Result);
      }
    } else {
      Result = LoadGPRRegister(gpr, OpSize, highIndex ? 8 : 0, AllowUpperGarbage);
    }
  }
  ...
  if (ShouldLoad) {
    ...
    bool AddBase = AddMemoryBase;
    if (Operand.IsLiteral() || Operand.IsLiteralRelocation()) {
      AddBase = false;
    }
    Result = LoadEffectiveAddress(this, A, GetGPROpSize(), false, AllowUpperGarbage, AddBase);
  }
