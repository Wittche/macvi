...
void SetDataDirectory(const std::string_view Path, bool Global) {
  fprintf(stderr, "SetDataDirectory: %.*s (Global: %d)\n", (int)Path.size(), Path.data(), Global);
...
void Initialize() {
  fprintf(stderr, "FEXCore::Config::Initialize: Starting\n");
  AddLayer(fextl::make_unique<MetaLayer>(FEXCore::Config::LayerType::LAYER_TOP));
  fprintf(stderr, "FEXCore::Config::Initialize: Layer added\n");
  Meta = dynamic_cast<MetaLayer*>(ConfigLayers.begin()->second.get());
  fprintf(stderr, "FEXCore::Config::Initialize: Meta assigned: %p\n", (void*)Meta);
}
...
fextl::string ExpandPath(const fextl::string& ContainerPrefix, const fextl::string& PathName) {
  fprintf(stderr, "FEXDBG ExpandPath: PathName='%s', ContainerPrefix='%s'\n", PathName.c_str(), ContainerPrefix.c_str());
  if (PathName.empty()) {
    fprintf(stderr, "FEXDBG ExpandPath: PathName is empty, returning {}\n");
    return {};
  }

  // Expand home if it exists
  if (FHU::Filesystem::IsRelative(PathName)) {
    fprintf(stderr, "FEXDBG ExpandPath: PathName is relative\n");
    ...
    fprintf(stderr, "FEXDBG ExpandPath: PathName is absolute\n");
    ...
    bool exists = FHU::Filesystem::Exists(PathName);
    fprintf(stderr, "FEXDBG ExpandPath: Exists('%s') = %d\n", PathName.c_str(), exists);
    if (exists) {
      return PathName;
    }
  }
  fprintf(stderr, "FEXDBG ExpandPath: returning empty\n");
  return {};
}
...
void ReloadMetaLayer() {
  fprintf(stderr, "FEXDBG ReloadMetaLayer: Starting\n");
  Meta->Load();
  fprintf(stderr, "FEXDBG ReloadMetaLayer: Meta->Load() done\n");

  // Debug: dump all options in the meta layer
  {
    bool rootfsExists = FEXCore::Config::Exists(FEXCore::Config::CONFIG_ROOTFS);
    fprintf(stderr, "FEXDBG ReloadMetaLayer: CONFIG_ROOTFS exists = %d\n", rootfsExists);
    if (rootfsExists) {
      auto val = Meta->Get(FEXCore::Config::CONFIG_ROOTFS);
      if (val) {
        fprintf(stderr, "FEXDBG ReloadMetaLayer: CONFIG_ROOTFS value = '%s'\n", (*val)->c_str());
      } else {
        fprintf(stderr, "FEXDBG ReloadMetaLayer: CONFIG_ROOTFS Get returned nullopt\n");
      }
    }
  }

  const fextl::string ContainerPrefix {FindContainerPrefix()};
  fprintf(stderr, "FEXDBG ReloadMetaLayer: ContainerPrefix = '%s'\n", ContainerPrefix.c_str());
  ...
  if (FEXCore::Config::Exists(FEXCore::Config::CONFIG_ROOTFS)) {
    ...
    fprintf(stderr, "FEXDBG ReloadMetaLayer: ROOTFS PathName = '%s'\n", PathName->c_str());
    const auto ExpandedString = ExpandPath(ContainerPrefix, *PathName);
    fprintf(stderr, "FEXDBG ReloadMetaLayer: ROOTFS ExpandedString = '%s'\n", ExpandedString.c_str());
    if (!ExpandedString.empty()) {
      // Adjust the path if it ended up being relative
      FEXCore::Config::Set(FEXCore::Config::CONFIG_ROOTFS, ExpandedString);
      fprintf(stderr, "FEXDBG ReloadMetaLayer: Set ROOTFS to '%s'\n", ExpandedString.c_str());
    } else if (!PathName->empty()) {
      fprintf(stderr, "FEXDBG ReloadMetaLayer: ROOTFS path not found, searching data/config dirs\n");
      ...
      for (auto Global : {true, false}) {
        for (auto DirectoryFetchers : {GetDataDirectory, GetConfigDirectory}) {
          ...
          fprintf(stderr, "FEXDBG ReloadMetaLayer: Checking '%s'\n", NamedRootFS.c_str());
          if (FHU::Filesystem::Exists(NamedRootFS)) {
            FEXCore::Config::Set(FEXCore::Config::CONFIG_ROOTFS, NamedRootFS);
            fprintf(stderr, "FEXDBG ReloadMetaLayer: Found and set ROOTFS to '%s'\n", NamedRootFS.c_str());
          }
        }
      }
    }
  } else {
    fprintf(stderr, "FEXDBG ReloadMetaLayer: CONFIG_ROOTFS does NOT exist in meta layer!\n");
  }
