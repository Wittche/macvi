...
#define foreach_bit(b, x) for (uint32_t __x = (x), b; ((b) = __builtin_ffs(__x) - 1, __x); __x &= ~(1 << (b)))
  ...
  void SpillReg(RegisterClassData* Class, IROp_CodeBlock* Block, IROp_Header* Exclude) {
    ...
    if (Candidate == nullptr) {
      LogMan::Msg::EFmt("SpillReg failed! Exclude Op: {}, NumArgs: {}, Class Count: {}, Class Available: {:b}",
                        IR::GetName(Exclude->Op),
                        IR::GetRAArgs(Exclude->Op),
                        Class->Count,
                        Class->Available);
      for (int s = 0; s < IR::GetRAArgs(Exclude->Op); ++s) {
        auto V = Exclude->Args[s];
        if (V.IsImmediate()) {
          PhysicalRegister PReg(V);
          LogMan::Msg::EFmt("  Arg {}: Immediate (Reg class {}, reg {})", s, (int)PReg.Class, (int)PReg.Reg);
        } else {
          auto Reg = SSAToReg[V.ID().Value];
          LogMan::Msg::EFmt("  Arg {}: SSA {} (Reg class {}, reg {})", s, V.ID().Value, (int)Reg.Class, (int)Reg.Reg);
        }
      }
    }
