...
#else
static struct { size_t page_size; } global_config = { 4096 };
...
void* memalign(size_t align, size_t s) {
  if (align <= 1) return ::malloc(s);
#ifdef __APPLE__
  void* ptr;
  if (align < sizeof(void*)) align = sizeof(void*);
  if (posix_memalign(&ptr, align, s) != 0)
    return nullptr;
  return ptr;
#else
...
void* valloc(size_t size) {
#ifdef __APPLE__
  return memalign(global_config.page_size, size);
#else
  return ::valloc(size);
#endif
}
...
void* aligned_alloc(size_t a, size_t s) {
  ...
  if (!ptr) {
    fprintf(stderr, "FEXDBG: Allocator::aligned_alloc(align=%zu, size=%zu) FAILED!\n", a, s);
    fflush(stderr);
  }
  return ptr;
}
