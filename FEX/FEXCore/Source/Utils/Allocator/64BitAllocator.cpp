...
private:
  ...
  LiveVMARegion* MakeRegionActive(ReservedRegionListType::iterator ReservedIterator, uint64_t UsedSize) {
    ...
    uintptr_t Base = ReservedRegion->Base;
    uint64_t RegionSize = ReservedRegion->RegionSize;

    ReservedRegions->erase(ReservedIterator);

    // mprotect the new region we've allocated
    size_t SizeOfLiveRegion =
      FEXCore::AlignUp(LiveVMARegion::GetFEXManagedVMARegionSize(RegionSize), FEXCore::Utils::FEX_PAGE_SIZE);
    size_t SizePlusManagedData = UsedSize + SizeOfLiveRegion;

#ifdef __APPLE__
    // macOS/ARM64 requires 16KB alignment for mprotect
    size_t HostPageSize = getpagesize();
    uintptr_t AlignedBase = Base & ~(HostPageSize - 1);
    size_t AlignedSize = FEXCore::AlignUp(SizePlusManagedData + (Base - AlignedBase), HostPageSize);
    auto Res = mprotect(reinterpret_cast<void*>(AlignedBase), AlignedSize, PROT_READ | PROT_WRITE);
#else
    auto Res = mprotect(reinterpret_cast<void*>(Base), SizePlusManagedData, PROT_READ | PROT_WRITE);
#endif
    LOGMAN_THROW_A_FMT(Res != -1, "Couldn't mprotect region: {} '{}' (Base: 0x{:x}, Size: {})", errno,
                       strerror(errno), Base, SizePlusManagedData);

    FEXCore::Allocator::VirtualName("FEXMem_Misc", reinterpret_cast<void*>(Base), SizePlusManagedData);
    LiveVMARegion* LiveRange = new (reinterpret_cast<void*>(Base)) LiveVMARegion();
  ...
  auto CheckIfRangeFits = [&AllocatedOffset](LiveVMARegion* Region, uint64_t length, int prot, int flags, int fd, off_t offset,
    ...
    if (Region->FreeSpace >= length) {
      ...
      if (AllocatedPage != ~0ULL) {
        ...
        int mmap_flags = (flags & ~MAP_FIXED_NOREPLACE) | MAP_FIXED;
#ifdef __APPLE__
        if (prot & PROT_EXEC) {
          mmap_flags |= MAP_JIT;
        }
#endif
        void* MMapResult = ::mmap(reinterpret_cast<void*>(AllocatedOffset), length, prot, mmap_flags, fd, offset);
...
fextl::unique_ptr<Alloc::HostAllocator> Create64BitAllocatorWithRegions(fextl::vector<FEXCore::Allocator::MemoryRegion>& Regions) {
  if (Regions.empty()) {
    LogMan::Msg::EFmt("No memory regions found to create 64-bit allocator!");
    return nullptr;
  }
