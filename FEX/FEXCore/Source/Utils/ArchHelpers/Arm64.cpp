...
#ifdef __APPLE__
  // On Apple Silicon, ldaxp requires write permission on some cores/page-types.
  // Since Apple Silicon supports FEAT_LSE2, an aligned ldp is atomic.
  // We use ldp + dmb ishld to get atomic load-acquire semantics without needing write permission.
  __asm volatile(
    R"(
  ldp %[ResultLower], %[ResultUpper], [%[Addr]];
  dmb ishld;
)"
...
#ifdef __APPLE__
  uint64_t Result;
  __asm volatile("ldar %[res], [%[addr]]" : [res] "=r"(Result) : [addr] "r"(Addr) : "memory");
  return Result;
#else
...
#ifdef __APPLE__
  uint32_t Result;
  __asm volatile("ldar %w[res], [%[addr]]" : [res] "=r"(Result) : [addr] "r"(Addr) : "memory");
  return Result;
#else
...
#ifdef __APPLE__
  uint8_t Result;
  __asm volatile("ldarb %w[res], [%[addr]]" : [res] "=r"(Result) : [addr] "r"(Addr) : "memory");
  return Result;
#else
...
static uint16_t LoadAcquire16(uint64_t Addr) {
#ifdef __APPLE__
  uint16_t Result;
  __asm volatile("ldarh %w[res], [%[addr]]" : [res] "=r"(Result) : [addr] "r"(Addr) : "memory");
  return Result;
#else
  auto Atom = std::atomic_ref<uint16_t>(*reinterpret_cast<uint16_t*>(Addr));
  return Atom.load(std::memory_order_acquire);
#endif
}
  ...
  if ((Addr & AlignmentMask) == 15) {
    ...
    if ((Addr & AlignmentMask) == 7) {
      ...
      if ((Addr & AlignmentMask) == 3) {
        ...
        uint64_t TmpResult = LoadAcquire64(Addr);
        ...
        uint32_t TmpResult = LoadAcquire32(Addr);
...
static void DoStore128(uint64_t Addr, __uint128_t Val) {
  uint64_t Lower = (uint64_t)Val;
  uint64_t Upper = (uint64_t)(Val >> 64);

#ifdef __APPLE__
  // On Apple Silicon, FEAT_LSE2 makes aligned stp atomic.
  __asm volatile(
    R"(
  dmb ish;
  stp %[ValLower], %[ValUpper], [%[Addr]];
  dmb ish;
)"
    :
    : [ValLower] "r"(Lower), [ValUpper] "r"(Upper), [Addr] "r"(Addr)
    : "memory");
#else
  __asm volatile(
    R"(
1:
  ldaxp xzr, xzr, [%[Addr]];
  stlxp w11, %[ValLower], %[ValUpper], [%[Addr]];
  cbnz w11, 1b;
)"
    :
    : [ValLower] "r"(Lower), [ValUpper] "r"(Upper), [Addr] "r"(Addr)
...
static bool RunCASPAL(uint64_t* GPRs, uint32_t Size, uint32_t DesiredReg1, uint32_t DesiredReg2, uint32_t ExpectedReg1,
  ...
  if (Size == 0) {
...
#ifdef __APPLE__
    Addr += FEXCore::Utils::GlobalMemoryBase;
#endif
    ...
    if ((Addr & AlignmentMask) > 8) {
      ...
      while (1) {
        TmpExpected = LoadAcquire128(Addr);
...
static bool HandleCASPAL(uint32_t Instr, uint64_t* GPRs, uint32_t* StrictSplitLockMutex) {
  ...
  uint32_t ExpectedReg1 = Instr & 0b11111;
  uint32_t ExpectedReg2 = ExpectedReg1 + 1;
  uint32_t DesiredReg1 = (Instr >> 16) & 0b11111;
...
static uint64_t HandleCASPAL_ARMv8(uint32_t Instr, uintptr_t ProgramCounter, uint64_t* GPRs, uint32_t* StrictSplitLockMutex) {
  ...
  if (Size == 1) {
    ...
    if (DataReg == 31) {
      ...
      if ((NextInstr & ArchHelpers::Arm64::STLXP_MASK) == ArchHelpers::Arm64::STLXP_INST) {
        uint64_t Addr = GPRs[AddrReg];
#ifdef __APPLE__
        Addr += FEXCore::Utils::GlobalMemoryBase;
#endif
        uint32_t StoreDataReg = NextInstr & 0x1F;
        uint32_t StoreDataReg2 = (NextInstr >> 10) & 0x1F;
        uint32_t StoreStatusReg = (NextInstr >> 16) & 0x1F;

        __uint128_t Val = GPRs[StoreDataReg2];
        Val <<= 64;
        Val |= GPRs[StoreDataReg];

        DoStore128(Addr, Val);
        GPRs[StoreStatusReg] = 0; // Success

        // Skip ldaxp and stlxp
  ...
  if ((Addr & AlignmentMask) == 15) {
    ...
    if ((Addr & AlignmentMask) == 7) {
      ...
      if ((Addr & AlignmentMask) == 3) {
        ...
        while (1) {
          TmpExpected = LoadAcquire64(Addr);
        ...
        if (Alignment == 0 || Alignment == 2) {
            // Aligned 16-bit CAS
            auto Atomic = std::atomic_ref<uint16_t>(*reinterpret_cast<uint16_t*>(Addr));
            while (1) {
              uint16_t Actual = LoadAcquire16(Addr);

              uint16_t Desired = DesiredFunction(Actual, DesiredSrc);
              uint16_t Expected = ExpectedFunction(Actual, ExpectedSrc);

              bool CASResult = Atomic.compare_exchange_strong(Actual, Desired);
              if (CASResult) {
                // Stored successfully
                return Expected;
              } else {
            ...
            // Unaligned (offset 1) within 4-byte block
            ...
            while (1) {
              TmpExpected = LoadAcquire32(Addr);
...
static uint32_t DoCAS32(uint32_t DesiredSrc, uint32_t ExpectedSrc, uint64_t Addr, CASExpectedFn<uint32_t> ExpectedFunction,
  ...
  if ((Addr & AlignmentMask) > 12) {
    ...
    if ((Addr & AlignmentMask) >= 5) {
      ...
      while (1) {
        __uint128_t TmpActual = LoadAcquire128(Addr);
      ...
      auto Atomic = std::atomic_ref<uint64_t>(*reinterpret_cast<uint64_t*>(Addr));
      while (1) {
        uint64_t TmpActual = LoadAcquire64(Addr);
...
static bool RunCASAL(uint64_t* GPRs, uint32_t Size, uint32_t DesiredReg, uint32_t ExpectedReg, uint32_t AddressReg, uint32_t* StrictSplitLockMutex) {
  ...
  Addr += FEXCore::Utils::GlobalMemoryBase;
#endif
  std::optional<uint64_t> Res = DoCAS(Size, GPRs[DesiredReg], GPRs[ExpectedReg], Addr, StrictSplitLockMutex);
  if (!Res.has_value()) {
    return false;
  }
...
static bool HandleCASAL(uint64_t* GPRs, uint32_t Instr, uint32_t* StrictSplitLockMutex) {
  ...
  uint32_t ExpectedReg = Instr & 0b11111;
  uint32_t DesiredReg = (Instr >> 16) & 0b11111;
  uint32_t AddressReg = (Instr >> 5) & 0b11111;
...
static uint64_t HandleAtomicLoadstoreExclusive(uintptr_t ProgramCounter, uint64_t* GPRs, uint32_t* StrictSplitLockMutex) {
  ...
  fprintf(stderr, "[FEX-HALE-DEBUG] PC=%p Instr=0x%08x ResultReg=%u AddressReg=%u Addr=0x%llx\n",
          PC, Instr, ResultReg, AddressReg, (unsigned long long)Addr);
  for (int j = 1; j < 6; ++j) {
    fprintf(stderr, "[FEX-HALE-DEBUG]   PC[%d] = 0x%08x\n", j, PC[j]);
  }
  fflush(stderr);
...
std::optional<int32_t> HandleUnalignedAccess(FEXCore::Core::InternalThreadState* Thread, UnalignedHandlerType HandleType,
  ...
  static bool DebugEnv = getenv("FEX_DEBUG_SIGBUS") != nullptr;
  if (DebugEnv) {
    fprintf(stderr, "FEXDBG: HandleUnalignedAccess PC=0x%llx Instr=0x%08x Type=%d IsJIT=%d\n", (unsigned long long)ProgramCounter, Instr, (int)HandleType, (int)IsJIT);
  }
  ...
  if (!IsJIT) [[unlikely]] {
    if ((Instr & LDAXR_MASK) == LDAR_INST ||  // LDAR*
      ...
      if (std::optional<uint64_t> Prev =
            DoCAS(Size, DataReg == 31 ? 0 : GPRs[DataReg], Thread->ExclusiveStore.Store, Addr, StrictSplitLockMutex)) {
        if (StatusReg != 31) {
  ...
  // fprintf(stderr, "[FEX-SIGBUS-DEBUG] Frame=%p, BlockBegin=0x%llx\n", Frame, (unsigned long long)BlockBegin);
  // fflush(stderr);
  if (BlockBegin == 0) [[unlikely]] {
    LogMan::Msg::EFmt("SIGBUS in JIT but InlineJITBlockHeader is null/stale. PC: 0x{:x} Instruction: 0x{:08x}", ProgramCounter, PC[0]);
    return std::nullopt;
  }
  auto InlineHeader = reinterpret_cast<const CPU::CPUBackend::JITCodeHeader*>(BlockBegin);
  // fprintf(stderr, "[FEX-SIGBUS-DEBUG] InlineHeader=%p, OffsetToBlockTail=0x%x\n", InlineHeader, InlineHeader ? InlineHeader->OffsetToBlockTail : 0);
  // fflush(stderr);
  ...
  // fprintf(stderr, "[FEX-SIGBUS-DEBUG] InlineTail=%p, SpinLockFutex=%d\n", InlineTail, InlineTail ? (int)InlineTail->SpinLockFutex : -1);
  // fflush(stderr);
  ...
  if ((Instr & ArchHelpers::Arm64::CASPAL_MASK) == ArchHelpers::Arm64::CASPAL_INST) { // CASPAL
    ...
    // Fall through to the spin-lock implementation below.
  } else if ((Instr & ArchHelpers::Arm64::ATOMIC_MEM_MASK) == ArchHelpers::Arm64::ATOMIC_MEM_INST) { // Atomic memory op
    if (ArchHelpers::Arm64::HandleAtomicMemOp(Instr, GPRs, StrictSplitLockMutex)) {
      // Skip this instruction now
      return 4;
...
#ifdef __APPLE__
  // macOS can deliver repeated SIGBUS/SIGILL for RCpc unaligned accesses while
  // trying to backpatch the same JIT block. Prefer direct emulation here so the
  // guest makes forward progress through early loader/bash startup.
  if ((Instr & LDAXR_MASK) == LDAR_INST ||
      (Instr & LDAXR_MASK) == LDAPR_INST) {
    if (ArchHelpers::Arm64::HandleAtomicLoad(Instr, GPRs, 0)) {
      return 4;
    }
    LogMan::Msg::EFmt("Unhandled JIT SIGBUS LDAR*: PC: 0x{:x} Instruction: 0x{:08x}\n", ProgramCounter, PC[0]);
    ...
    LogMan::Msg::EFmt("Unhandled JIT SIGBUS STLUR*: PC: 0x{:x} Instruction: 0x{:08x}\n", ProgramCounter, PC[0]);
    return std::nullopt;
  }

  // Signal-safe lock for macOS to avoid deadlocks with WFE in signal handlers
  struct SignalSafeUniqueSpinMutex {
    SignalSafeUniqueSpinMutex(uint32_t* Futex) : Futex(Futex) {
      auto AtomicFutex = std::atomic_ref<uint32_t>(*Futex);
      uint32_t Expected = 0;
      while (!AtomicFutex.compare_exchange_weak(Expected, 1, std::memory_order_acquire)) {
        Expected = 0;
        __builtin_arm_isb(0b1111); // isb sy
      }
    }
    ~SignalSafeUniqueSpinMutex() {
      std::atomic_ref<uint32_t>(*Futex).store(0, std::memory_order_release);
    }
    uint32_t* Futex;
  };
  SignalSafeUniqueSpinMutex lk(&InlineTail->SpinLockFutex);
#else
...
#endif
  FEXCore::Allocator::JITWriteScope scope;
  ...
  auto AtomicInst = LoadAcquire32(reinterpret_cast<uintptr_t>(&PC[0]));
  if ((AtomicInst & LDSTREGISTER_MASK) == LDR_INST || (AtomicInst & LDSTUNSCALED_MASK) == LDUR_INST) {
    ...
    if (HandleType != UnalignedHandlerType::NonAtomic) {
      ...
      auto DMBInst = LoadAcquire32(reinterpret_cast<uintptr_t>(&PC[1]));
      if (DMBInst == DMB_LD) {
        return 0;
      }
    } else {
    ...
    // - PC[1] = STP
    // - PC[2] = DMB
    auto STPInst = LoadAcquire32(reinterpret_cast<uintptr_t>(&PC[1]));
    auto DMBInst = LoadAcquire32(reinterpret_cast<uintptr_t>(&PC[2]));
...
#endif // defined(__aarch64__) || defined(_M_ARM_64)
} // namespace FEXCore::ArchHelpers::Arm64
