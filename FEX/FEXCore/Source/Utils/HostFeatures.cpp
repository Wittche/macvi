...
#ifdef __APPLE__
  // macOS doesn't have MIDR via sysfs
  Features->CPUMIDRs.resize(FEX::CPUInfo::CalculateNumberOfCPUs(), 0);
#else
...
#if defined(ARCHITECTURE_arm64) && !defined(VIXL_SIMULATOR) && !defined(__APPLE__)
...
#ifdef ARCHITECTURE_arm64
#ifdef __APPLE__
static uint64_t Get_MIDR_EL1() {
  uint64_t val = 0;
  size_t size = sizeof(val);
  if (sysctlbyname("hw.cpufamily", &val, &size, nullptr, 0) == 0) return val;
  return 0;
}
#else
...
#endif
#define GetDCZID_EL0() 0

class CPUFeaturesFromID final : public FEX::CPUFeatures {
public:
  CPUFeaturesFromID() {
#ifdef __APPLE__
    // Apple Silicon features via sysctl
    auto GetSysctl = [](const char* name) -> int {
      int val = 0;
      size_t size = sizeof(val);
      if (sysctlbyname(name, &val, &size, nullptr, 0) == 0) return val;
      return 0;
    };
    ...
    // ISAR0
    uint64_t aes = 0;
    if (GetSysctl("hw.optional.arm.FEAT_AES")) aes = 1;
    if (GetSysctl("hw.optional.arm.FEAT_PMULL")) aes = 2;
    isar0 |= (aes << 4);

    if (GetSysctl("hw.optional.arm.FEAT_SHA1")) isar0 |= (1ULL << 8);

    uint64_t sha2 = 0;
    if (GetSysctl("hw.optional.arm.FEAT_SHA256")) sha2 = 1;
    if (GetSysctl("hw.optional.arm.FEAT_SHA512")) sha2 = 2;
    isar0 |= (sha2 << 12);

    if (GetSysctl("hw.optional.arm.FEAT_CRC32")) isar0 |= (1ULL << 16);
    if (GetSysctl("hw.optional.arm.FEAT_LSE")) isar0 |= (2ULL << 20);
    if (GetSysctl("hw.optional.arm.FEAT_RDM")) isar0 |= (1ULL << 28);
    if (GetSysctl("hw.optional.arm.FEAT_SHA3")) isar0 |= (1ULL << 32);
    if (GetSysctl("hw.optional.arm.FEAT_DotProd")) isar0 |= (1ULL << 44);
    if (GetSysctl("hw.optional.arm.FEAT_FHM")) isar0 |= (1ULL << 48);

    uint64_t ts = 0;
    if (GetSysctl("hw.optional.arm.FEAT_FlagM")) ts = 1;
    if (GetSysctl("hw.optional.arm.FEAT_FlagM2")) ts = 2;
    isar0 |= (ts << 52);

    // ISAR1
    uint64_t dpb = 0;
    if (GetSysctl("hw.optional.arm.FEAT_DPB")) dpb = 1;
    if (GetSysctl("hw.optional.arm.FEAT_DPB2")) dpb = 2;
    isar1 |= dpb;

    if (GetSysctl("hw.optional.arm.FEAT_JSCVT")) isar1 |= (1ULL << 12);
    if (GetSysctl("hw.optional.arm.FEAT_FCMA")) isar1 |= (1ULL << 16);

    uint64_t lrcpc = 0;
    if (GetSysctl("hw.optional.arm.FEAT_LRCPC")) lrcpc = 1;
    if (GetSysctl("hw.optional.arm.FEAT_LRCPC2")) lrcpc = 2;
    isar1 |= (lrcpc << 20);

    if (GetSysctl("hw.optional.arm.FEAT_FRINTTS")) isar1 |= (1ULL << 32);
    if (GetSysctl("hw.optional.arm.FEAT_SB")) isar1 |= (1ULL << 36);

    uint64_t bf16 = 0;
    if (GetSysctl("hw.optional.arm.FEAT_BF16")) bf16 = 1;
    if (GetSysctl("hw.optional.arm.FEAT_SME_F64F64")) bf16 = 2;
    isar1 |= (bf16 << 44);

    if (GetSysctl("hw.optional.arm.FEAT_I8MM")) isar1 |= (1ULL << 52);

    // PFR0
    uint64_t fp = 0;
    if (GetSysctl("hw.optional.arm.FEAT_FP16")) fp = 1;
    pfr0 |= (fp << 16);

    uint64_t asimd = 0;
    if (GetSysctl("hw.optional.arm.AdvSIMD_HPFPCvt")) asimd = 1;
    pfr0 |= (asimd << 20);

    if (GetSysctl("hw.optional.arm.FEAT_DIT")) pfr0 |= (1ULL << 48);
    if (GetSysctl("hw.optional.arm.FEAT_CSV2")) pfr0 |= (1ULL << 56);
    if (GetSysctl("hw.optional.arm.FEAT_CSV3")) pfr0 |= (1ULL << 60);

    // PFR1
    if (GetSysctl("hw.optional.arm.FEAT_BTI")) pfr1 |= 1ULL;
    if (GetSysctl("hw.optional.arm.FEAT_SSBS")) pfr1 |= (1ULL << 4);

    uint64_t sme = 0;
    if (GetSysctl("hw.optional.arm.FEAT_SME")) sme = 1;
    if (GetSysctl("hw.optional.arm.FEAT_SME2")) sme = 2;
    pfr1 |= (sme << 24);

    // MMFR0
    if (GetSysctl("hw.optional.arm.FEAT_ECV")) mmfr0 |= (2ULL << 60);

    // MMFR1
    if (GetSysctl("hw.optional.arm.FEAT_AFP")) mmfr1 |= (1ULL << 44);

    // MMFR2
    if (GetSysctl("hw.optional.arm.FEAT_LSE2")) mmfr2 |= (1ULL << 32);
...
#endif
    DCZID.SetReg(0b1'0000); // Unsupported DCZVA for now
...
#define ENABLE_DISABLE_OPTION(FeatureName, name, enum_name)                                                                        \
  do {                                                                                                                             \
    ...
    if (Disable##name && Enable##name) {                                                                                           \
      LogMan::Msg::EFmt("Disabling and Enabling CPU feature (" #name ") is mutually exclusive. Defaulting to Disable.");           \
    }                                                                                                                              \
...
#ifdef ARCHITECTURE_arm64
#ifdef __APPLE__
  // CTR_EL0 is not directly readable on macOS userspace
  // MIDR_EL1 is not directly readable on macOS userspace
  // We can get some of this via sysctl
  MIDR = Get_MIDR_EL1();
#else
