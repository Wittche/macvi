...
inline void* VirtualAlloc(size_t Size, bool Execute = false, bool Commit = true) {
  ...
  // On macOS with MAP_JIT, the page is writable by default after mmap.
  // DO NOT call pthread_jit_write_protect_np(true) here, as it would prevent code emission.
  return result;
#else
...
inline void* VirtualAlloc(void* Base, size_t Size, bool Execute = false, bool Commit = true) {
  ...
  void* result = ::mmap(Base, Size, prot, flags, -1, 0);
  // On macOS with MAP_JIT, the page is writable by default after mmap.
  // DO NOT call pthread_jit_write_protect_np(true) here, as it would prevent code emission.
  return result;
#else
...
#ifdef __APPLE__
  // macOS doesn't support MADV_HUGEPAGE/NOHUGEPAGE in this way.
  // It uses VM_FLAGS_SUPERPAGE at mmap time or other mechanisms.
#else
