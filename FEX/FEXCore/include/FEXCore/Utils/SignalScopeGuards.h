...
public:
...
#if defined(__APPLE__)
    HasOriginal_Apple = true;
    sigset_t new_mask;
    // This assumes uint64_t Mask fits in sigset_t or we only care about the first 64 signals.
    // On macOS, sigset_t is usually a single 32-bit integer or a small struct.
    // We'll use a conservative approach.
    sigfillset(&new_mask); // Mask everything for now as a safe default for ~0ULL
    pthread_sigmask(SIG_SETMASK, &new_mask, &OldMask_Apple);
    // We need to store old_mask. For simplicity on macOS, we might need a different storage.
    // However, FEX uses uint64_t for mask storage.
    // Let's use sigset_t storage if we are on Apple.
#else
...
#if defined(__APPLE__)
    OldMask_Apple = rhs.OldMask_Apple;
    HasOriginal_Apple = rhs.HasOriginal_Apple;
    rhs.HasOriginal_Apple = false;
#else
  ...
  ~ScopedSignalMasker() {
#if defined(__APPLE__)
    if (HasOriginal_Apple) {
      pthread_sigmask(SIG_SETMASK, &OldMask_Apple, nullptr);
    }
#else
    if (OriginalMask) {
...
private:
#if defined(__APPLE__)
  sigset_t OldMask_Apple;
  bool HasOriginal_Apple {true};
#else
