...
public:
  ...
  /**
   * @brief Checks if a PC is inside the shared dispatcher code buffer.
   *
   * @param Address The PC to check against.
   *
   * @return true if PC is inside the dispatcher's code buffer.
   */
  FEX_DEFAULT_VISIBILITY virtual bool IsAddressInDispatcher(uintptr_t Address) const = 0;
