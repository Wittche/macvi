...
struct ELFParser {
  ...
  bool ReadElf(int NewFD) {
    fprintf(stderr, "FEXDBG: ReadElf(fd=%d) starting\n", NewFD);
    Closefd();
    ...
    if (fd == -1) {
      fprintf(stderr, "FEXDBG: ReadElf: fd is -1, returning false\n");
      return false;
    }
    ...
    if (Size < 4) {
      fprintf(stderr, "FEXDBG: ReadElf: file size %lld too small\n", (long long)Size);
      return false;
    }

    // Reset to beginning
    if (lseek(fd, 0, SEEK_SET) == -1) {
      fprintf(stderr, "FEXDBG: ReadElf: lseek failed\n");
      return false;
    }
    ...
    if (pread(fd, header, sizeof(header), 0) == -1) {
      ...
      fprintf(stderr, "FEXDBG: ReadElf: pread header failed\n");
      return false;
    }
    ...
    if (header[0] != ELFMAG0 || header[1] != ELFMAG1 || header[2] != ELFMAG2 || header[3] != ELFMAG3) {
      ...
      fprintf(stderr, "FEXDBG: ReadElf: MAGIC mismatch: %02x %02x %02x %02x\n", header[0], header[1], header[2], header[3]);
      return false;
    }
    
    fprintf(stderr, "FEXDBG: ReadElf: MAGIC OK, class=%d\n", header[EI_CLASS]);
...
#undef COPY
  ...
  bool ReadElf(const fextl::string& file) {
    ...
    fprintf(stderr, "FEXDBG: ReadElf(path=%s) open_fd=%d\n", file.c_str(), NewFD);
