...
#else
  // Safe default for Apple Silicon (M1+)
  Result = (2ULL << 20); // LSE enabled
#endif
  return Result;
}
...
int main(int argc, char** argv, char** envp) {
  fprintf(stderr, "FEXGetConfig: Starting\n");
  FEX::Config::InitializeConfigs(FEX::Config::PortableInformation {});
  fprintf(stderr, "FEXGetConfig: InitializeConfigs done\n");
  FEXCore::Config::Initialize();
  fprintf(stderr, "FEXGetConfig: Initialize done\n");
  FEXCore::Config::AddLayer(FEX::Config::CreateGlobalMainLayer());
  fprintf(stderr, "FEXGetConfig: Global layer added\n");
  FEXCore::Config::AddLayer(FEX::Config::CreateMainLayer());
  fprintf(stderr, "FEXGetConfig: Main layer added\n");
  // No FEX arguments passed through command line
  FEXCore::Config::AddLayer(FEX::Config::CreateEnvironmentLayer(envp));
  fprintf(stderr, "FEXGetConfig: Env layer added\n");
  ...
  if (Options.is_set_by_user("version")) {
    fprintf(stdout, "FEX-Emu version 1.0 (macOS Port Debug)\n");
  }
  ...
  if (Options.is_set_by_user("install_prefix")) {
    ...
    auto Result = _NSGetExecutablePath(SelfPath, &size) == 0 ? strlen(SelfPath) : -1;
#else
