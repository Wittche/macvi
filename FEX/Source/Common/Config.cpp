...
fextl::string GetHomeDirectory() {
  fprintf(stderr, "GetHomeDirectory: Starting\n");
  const char* HomeDir = getenv("HOME");
  fprintf(stderr, "GetHomeDirectory: env HOME: %p (%s)\n", (void*)HomeDir, HomeDir ? HomeDir : "NULL");
...
fextl::string GetDataDirectory(bool Global, const PortableInformation& PortableInfo) {
  fprintf(stderr, "GetDataDirectory: Starting (Global: %d)\n", Global);
...
void InitializeConfigs(const PortableInformation& PortableInfo) {
  fprintf(stderr, "InitializeConfigs: Starting\n");
  auto data_local = GetDataDirectory(false, PortableInfo);
  fprintf(stderr, "InitializeConfigs: data_local: %s\n", data_local.c_str());
  FEXCore::Config::SetDataDirectory(data_local, false);

  auto data_global = GetDataDirectory(true, PortableInfo);
  fprintf(stderr, "InitializeConfigs: data_global: %s\n", data_global.c_str());
  FEXCore::Config::SetDataDirectory(data_global, true);

  auto config_local = GetConfigDirectory(false, PortableInfo);
  fprintf(stderr, "InitializeConfigs: config_local: %s\n", config_local.c_str());
  FEXCore::Config::SetConfigDirectory(config_local, false);

  auto config_global = GetConfigDirectory(true, PortableInfo);
  fprintf(stderr, "InitializeConfigs: config_global: %s\n", config_global.c_str());
  FEXCore::Config::SetConfigDirectory(config_global, true);

  auto file_local = GetConfigFileLocation(false, PortableInfo);
  fprintf(stderr, "InitializeConfigs: file_local: %s\n", file_local.c_str());
  FEXCore::Config::SetConfigFileLocation(file_local, false);

  auto file_global = GetConfigFileLocation(true, PortableInfo);
  fprintf(stderr, "InitializeConfigs: file_global: %s\n", file_global.c_str());
  FEXCore::Config::SetConfigFileLocation(file_global, true);
}
} // namespace FEX::Config
