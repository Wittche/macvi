...
OptionParser::OptionParser() :
  ...
  fprintf(stderr, "OptionParser: Constructor\n");
}
...
Values& OptionParser::parse_args(const int argc, char const* const* const argv) {
  fprintf(stderr, "parse_args: argc=%d, argv[0]=%s\n", argc, argv[0]);
  if (prog() == "") {
    fprintf(stderr, "parse_args: Setting prog\n");
    prog(basename(argv[0]));
    fprintf(stderr, "parse_args: Setting prog done\n");
  }

  fprintf(stderr, "parse_args: emplace_back starting\n");
  _parsed.emplace_back(argv[0]);
  fprintf(stderr, "parse_args: emplace_back done\n");
  return parse_args(&argv[1], &argv[argc]);
}
Values& OptionParser::parse_args(const fextl::vector<fextl::string>& v) {
  fprintf(stderr, "parse_args: vector size=%zu\n", v.size());
  _remaining.assign(v.begin(), v.end());
  fprintf(stderr, "parse_args: assigned to remaining\n");

  if (add_help_option()) {
    fprintf(stderr, "parse_args: adding help option\n");
    add_option("-h", "--help") .action("help") .help(_("show this help message and exit"));
    fprintf(stderr, "parse_args: adding help option done\n");
    _opts.splice(_opts.begin(), _opts, --(_opts.end()));
    fprintf(stderr, "parse_args: splice done\n");
  }
  ...
  while (not _remaining.empty()) {
    const fextl::string arg = _remaining.front();
    fprintf(stderr, "parse_args: processing arg: %s\n", arg.c_str());
    _remaining.pop_front();
    _leftover.push_back(arg);
  }
